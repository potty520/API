...
rem ============================================================
rem  JSON 接入系统 一键启动 (Windows)
rem  流程: 专用 MySQL(3307) -> 定位 Java 17 -> 必要时构建 -> 启动应用 -> 健康检查 -> 打开浏览器
...
if errorlevel 1 (
  echo [ERROR] 需要 Java 17, 当前为: %JAVA_EXE%
  exit /b 1
)

rem ==== 运行参数: 元数据库与种子目标库都指向本目录的专用 MySQL(3307) ====
set "META_DB_URL=jdbc:mysql://127.0.0.1:3307/json_ingestion?createDatabaseIfNotExist=true&useUnicode=true&characterEncoding=utf8&serverTimezone=Asia/Shanghai&allowPublicKeyRetrieval=true&useSSL=false"
...
echo 正在启动应用 http://127.0.0.1:3100 ...
